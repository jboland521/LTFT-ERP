## LTFT.ERP_tutorial.R
#############################################################################
## Description: A step-by-step implementation of the LTFT-ERP algorithm and the   
## associated procedures described in "A Study of Longitudinal Trends in Time-Frequency 
## Transformations of EEG Data during a Learning Experiment" by Boland et al. (2021). 
## This procedure assumes that the data is densely observed in the functional dimension.
#############################################################################
...
list.of.packages <- c("data.table", "rio", "splines", "viridis", "ggplot2", "ztable")
...
load("LTFT.ERP_simulation_fixed.effects.Rda") # Fixed effects estimates (list)
load("LTFT.ERP_simulation_random.effects.Rda") # Random effects estimates (list)
load("LTFT.ERP_simulation_X.Mean.csv") # Average power vector (vector)
load("LTFT.ERP_simulation_Phi.csv") # Estimated leading eigenvectors (matrix)
load("LTFT.ERP_simulation_Sigma.csv") # Estimated marginal covariance (matrix)
MissProf <- as.data.frame(fread("MissingProfileSim.csv")) # Missingness profile
...
cgroup <- c("", "Mean Power", "Eigenvector", "MDPCA Scores")
n.cgroup=c(2, 1, 1, 2)
tab1 <- ztable(ME.PE.Total, caption = "Overall ME and PE Estimates for a Simulation Run",
               align = "lllcccc")%>%
...
ME.PE.Group[, 1:2] <- floor(ME.PE.Group[, 1:2])
tab2 <- ztable(ME.PE.Group)
tab2
...
plot(group, condition, lobe, s, MEmodel, legend, n.spline)
